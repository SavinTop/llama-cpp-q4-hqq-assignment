...
void ggml_vec_dot_q4_hqq_q8_0_generic(int n, float * GGML_RESTRICT s, size_t bs, const void * GGML_RESTRICT vx, size_t bx, const void * GGML_RESTRICT vy, size_t by, int nrc) {
    ...
    for (int i = 0; i < nb; ++i) {
        ...
        for (int j = 0; j < QK8_0/2; ++j) {
            const int q4_lo = (x[i].qs[j] & 0x0F);
            const int q4_hi = (x[i].qs[j] >>   4);

            sumi  += q4_lo * y[i].qs[j];
            sumi  += q4_hi * y[i].qs[j + QK8_0/2];
            sumq8 += y[i].qs[j] + y[i].qs[j + QK8_0/2];
        }
...
static void ggml_vec_dot_q4_hqq_q8_0_nrc2(
    ...
    for (int i = 0; i < nb; ++i) {
        ...
        const bool zero0 = x0[i].scale == 0 && x0[i].zero == 0 && _mm_testz_si128(packed0, packed0);
        const bool zero1 = x1[i].scale == 0 && x1[i].zero == 0 && _mm_testz_si128(packed1, packed1);
        if (zero0 && zero1) {
            continue;
        }
        ...
        if (!zero0) {
            ...
            const __m256 zero_0 = _mm256_set1_ps(GGML_CPU_FP16_TO_FP32(x0[i].zero));
        ...
        if (!zero1) {
            ...
            const __m256 zero_1 = _mm256_set1_ps(GGML_CPU_FP16_TO_FP32(x1[i].zero));
